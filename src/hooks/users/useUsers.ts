...
import { userRoutePaths } from "../../router/navigation/paths";
import type {
  CreateUserPayload,
  UpdateUserPayload,
  User,
...
export function useUsers(): UseUsersResult {
  ...
  // page from `users` — leaving the edit form, which reads its record from that
  // array, blank. Off the list route we still fetch once (a deep link / refresh
  // straight onto a form), but never again.
  const isListRoute = location.pathname === userRoutePaths.list;
