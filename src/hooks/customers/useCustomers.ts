...
import { customerRoutePaths } from "../../router/navigation/paths";
import {
  type Customer,
...
export function useCustomers(): UseCustomersResult {
  ...
  // page from `customers` — leaving the edit form, which reads its record from
  // that array, blank. Off the list route we still fetch once (a deep link /
  // refresh straight onto a form), but never again.
  const isListRoute = location.pathname === customerRoutePaths.list;
